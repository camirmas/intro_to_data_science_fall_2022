...
begin
	download(url, "bta.csv")
	trial = CSV.read("bta.csv", DataFrame)
end
...
group_proportions = combine(groupby(trial, "Group"), "Result" => mean => "Fraction with pain relief")
...
begin
	...
	ax = Axis(fig[1, 1], 
		xlabel="Group", 
		ylabel="Fraction with pain relief",
		xticks=([1, 2], group_proportions[:, "Group"]),
		yticks=(0:.2:1))
	barplot!([1, 2], group_proportions[:, "Fraction with pain relief"])
	ylims!(0, 1)
...
md"
_null hypothesis_: The difference between treatment and control groups is due to chance. The treatment is not effective.

_alternative hypothesis_: The difference between treatment and control groups is _not_ due to chance. The treatment has an effect.
"
...
md"
_test statistic_:= `|proportion feeling relief in treatment - proportion feeling relief in control|`
...
transform!(trial, "Group" => Random.shuffle => "Sim Group under Null")
...
function proportion_w_pain_relief(data::DataFrame, group_col::String, group::String)
	filtered_group = filter(row -> row[group_col] == group, data)
	return mean(filtered_group[:, "Result"])
end
...
@assert proportion_w_pain_relief(trial, "Group", "Treatment") == .6

# ╔═╡ 18fd84e0-0e57-11eb-36a6-c72f5f1db6a6
@assert proportion_w_pain_relief(trial, "Group", "Control") == .125
...
function test_statistic(data::DataFrame, group_col::String)
	return abs(proportion_w_pain_relief(data, group_col, "Treatment") - proportion_w_pain_relief(data, group_col, "Control"))
end
...
# ╔═╡ e7b999ed-c179-4439-82db-3ac59f4afb5f
function run_simulation(data::DataFrame)
	shuffled_data = transform(data, "Group" => Random.shuffle => "Sim Group under Null")
	return test_statistic(shuffled_data, "Sim Group under Null")
end
...
begin
	...
	for i=1:N_simulations
		results[i] = run_simulation(trial)
	end
	results
end
...
begin
	baseline = test_statistic(trial, "Group")
	...
	ax2 = Axis(fig2[1, 1],
		...
		ylabel="Occurrences",
		xticks=0:.1:1)
	hist!(results; bins=0:.1:.7)
	vlines!(ax2, baseline, color = :red)

	fig2
end

# ╔═╡ 178c6c9c-22d3-4686-b615-69328e034797
begin
	p_value = (filter(r -> r >= baseline, results) |> length) / N_simulations
end
...
md"""
The p-value in this scenario indicates the likelihood that the observed effectiveness of the treatment can be described by the null hypothesis (i.e. that the treatment has no effect and that the observed difference is due to chance).
"""
...
In this case, because the p-value is less than 1% (highly statistically significant), we can reject the null hypothesis, and say that the treatment _does_ have an effect. 

🐶 what level of significance are you using, and why?

Because the p-value is less than 1%, the outcome is highly statistically significant.
...
This is a two-sided hypothesis because it asserts that the treatment has an effect, but it does not specify that the effect is good. Especially in pharmaceutical studies, it is important to know whether the treatment helps, and also whether it makes the treated condition worse.
...
Yes, the outcome indicates causality because it utilizes randomized control, it avoids confounding variables correlated to the null/alternative hypotheses that would alter the results.
...
This would cause confounding variables because patients with back pain will likely self-select into the treatment group, leading to results that favor the alternative hypothesis, but where the level of preexisting back pain influences the outcome.
"
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "cf8ec2d4432bce441b506294061fc842431ea17b"
